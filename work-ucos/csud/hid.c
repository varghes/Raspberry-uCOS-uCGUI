	...
	for (u32 i = 0; i < report->FieldCount; i++) {
		...
		if (field->Attributes.Variable) {
			if (field->LogicalMinimum < 0)
				field->Value.ST32 = BitGetSigned(report->ReportBuffer, field->Offset, field->Size);
			else
				field->Value.UT32 = BitGetUnsigned(report->ReportBuffer, field->Offset, field->Size);
		} else {
		...
		while (fields->count > 0) {
			...
			*(u32*)&report->Fields[report->FieldCount].Unit = *(u32*)&fields->unit;
			...
			if (report->Fields[report->FieldCount].Attributes.Variable) {
				...
				report->Fields[report->FieldCount].Value.UT32 = 0;
			}
			else {
				fields->count = 0;
